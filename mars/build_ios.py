...
def build_ios(tag=''):
    ...
    print('==================Output========================')
    print("os_framework: %s" % os_framework)
    print("simulator_framework: %s" % simulator_framework)

    return [os_framework,simulator_framework]
...
def build_ios_with_type(num):
    if num == '1':
        return build_ios()
    if num == '2':
        return build_ios_xlog()
    elif num == '3':
...
def main():
    if len(sys.argv) >= 2:
            build_ios_with_type(sys.argv[1])
    else:
        num = input('Enter menu:\n1. Clean && build mars.\n2. Clean && build xlog.\n3. Gen iOS mars Project.\n4. Exit\n')
        build_ios_with_type(num=num) 
